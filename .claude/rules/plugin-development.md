...
paths:
  - plugins/**
description: Claude Code プラグイン開発の仕様とベストプラクティス
---
...
```json
{
  ...
  "skills": "./skills/",           // agents は自動発見（plugin.json での指定は非推奨）
...
- `name` は kebab-case の識別子（Claude Code 上は `name` がスキル識別子であり、ディレクトリ名との一致は強制されない）
- **原則**: `name` とディレクトリ名は一致させる（例: `skills/commit/SKILL.md` → `name: commit`）
- **例外（プラグイン内プレフィックス許容）**: 汎用的な短い名前（`commit`, `push`, `merge`, `validation` 等）が他プラグインのスキル名と衝突する恐れがある場合、プラグイン名由来の prefix を付けて `name` を一意化してよい
  - 例: `git-actions/skills/commit/SKILL.md` → `name: git-commit`
  - 例: `plugin-generator/skills/validation/SKILL.md` → `name: plugin-validation`
- prefix を使う場合は当該プラグインの `CLAUDE.md` / `AGENTS.md` にディレクトリ名と `name` の対応表を明記すること

### 本文構造のパターン

スキルの用途に応じて次の 2 パターンを許容する:

1. **アクションスキル（標準）**: 手順を実行するスキル。`## Instructions` と `## Examples` を必ず持つ
2. **知識 / リファレンススキル（例外）**: 発動時に特定ドメインの知識をメインコンテキストへ読み込ませることを目的とするスキル。以下の条件をすべて満たす場合、独自の見出し構成（概要 / 用語 / API / ベストプラクティス 等）を許容する
   - `description` に "Use when user asks about ..." 等、知識提供トリガーを明記
   - `context: fork` または `user-invocable: false` のいずれかを設定（メインコンテキスト汚染防止、または裏参照専用化）
   - フロントマターに `allowed-tools` を指定し、書き換え系ツールを含めない（Read/Grep/Glob 中心）
   - スキル冒頭で「このスキルは知識提供型である」旨を明示

   **代表例**: `cloudflare-knowledge/*`, `*-cli-spec/*-cli-knowledge`, `gemini-api-spec/*`, `agent-browser-spec/*`, `nano-banana-image/image-generation`, `web-search-unified/unified-search` 等
...
`agent` フィールドでビルトインエージェント（Explore, Plan）またはカスタムエージェントを指定可能。
