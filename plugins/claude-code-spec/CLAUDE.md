...
**動的コンテキスト埋め込み記法の仕様を明確化:**
- Bash コマンド実行（`!`プレフィックス）: スラッシュコマンドのみ有効であることを明記
- スキルファイル内での使用は不可（エラーが発生）
- ファイル参照（`@`プレフィックス）との違いを説明

**理由:**
ユーザーがスキルファイル内で誤用してエラーを招かないよう、仕様をより明確に説明する必要があった。
実装者が正しく理解し、正確な仕様情報を提供できるようにするため。
