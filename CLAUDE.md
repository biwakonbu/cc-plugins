...
└── plugins/              # プラグイン格納ディレクトリ
    ...
    ├── memory-optimizer/   # メモリ管理最適化 (v1.1.1)
...
| memory-optimizer | 1.1.1 | Claude Code のメモリ管理（CLAUDE.md、rules、imports）を最適化するための知識とワークフローを提供するプラグイン |
| plugin-generator | 1.2.2 | Claude Code プラグインのスキャフォールディングとバリデーション |
